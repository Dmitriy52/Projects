let questions = [
	...
	
	{
        ...
        right: 1,
        answers: ["your father is doing","is your father doing","do your father do","your father do"],
    },
	...
	
	{
        ...
        right: 0,
        answers: ["am not watching","don’t watch","don’t watching","amn`t watching"],
    },
	...
	
	{
		text: "… in ghosts?",
        right: 1,
        answers: ["Are you believing","Do you believe","Are you believe","You don’t believe"],
    },
...
function nextPreviousQuestion (currentShownQuestionId,shownRadioButtons) {
	...
	if (currentShownQuestionId == lastQuestion){
		...
		//buttonCheckAnswers.style.display = "none";
	}
	...
	for (answer of questionToShow.answers) {
		...
		//if (usersAnswers[questionId] == i) {radioButton.checked = true}
		//i++
...
function findMissAnswers(cancelled) {
	...
	console.log(questions.length);
		...
		if (size != questions.length) {
			...
			mainContainer.appendChild(noticeMessаgeWindow);
			...
			noticeMessage.textContent = "У вас есть пропущенные вопросы, вы уверены что хотите продолжить?";
			...
			answerButtonYes.addEventListener("click", function() {popUpScore()} );
			answerButtonNo.addEventListener("click", function() {findMissAnswers(0); disabledElements(0)} );
...
function popUpScore () {
	...
	//console.log(score);
		...
		popUpText.textContent = "Вы ответили правильно на " + score + " вопрос (-а, -ов)";
		...
		butPopOk.addEventListener("click", function() {disabledElements(0); checkAnswers()} );	
...
function checkAnswers () {
	...
	mainContainer.textContent = "";
		...
		//mainContainer.appendChild(green);
	 ...
	 questions.forEach(function(question,number){
		 ...
		 answerText.textContent = "Правильный ответ: " +  question.answers[question.right];
		 ...
		 let imqLeftArrow = document.getElementById("leftArrow");
		 imqLeftArrow.style.display = "none";
		 
		 let imgRightArrow = document.getElementById("rightArrow");
		 ...
		 testWindowForm.appendChild(butBackToIndex);
		 	...
		 	if (question.right == usersAnswers[number]) {
				...
				answerText.textContent = "Вы ответили верно: " + question.answers[question.right];
						...
						answerComment.textContent += " Вы не ответили на данный вопрос ";
									...
									qTextAndImg.appendChild(red);
									red.style.display = "inline";
								answerComment.textContent += " Ваш ответ: " + question.answers[usersAnswers[number]];
				}
...
//let buttonStartTest = document.querySelector("#buttonStartTest");
//buttonStartTest.addEventListener("click", function() {startTest()} );
...
buttonNextQuestion.addEventListener("click", function() {сatchAnswers(); nextPreviousQuestion(++questionId)} );

let buttonPreviousQuestion = document.querySelector("#buttonPreviousQuestion");
buttonPreviousQuestion.addEventListener("click", function() {сatchAnswers(); nextPreviousQuestion(--questionId)} );

let buttonCheckAnswers = document.querySelector("#buttonCheckAnswers");
buttonCheckAnswers.addEventListener("click", function() {сatchAnswers(); findMissAnswers(1);} );
startTest();