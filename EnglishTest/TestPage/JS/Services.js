...
let scanFormInt = setInterval(buttonNextUnlock, 10);
...
function findSumsElem() {
  ...
  lessonsInput.oninput = function () {
    if (lessonsInput.value == 2) {
      ...
      lessonsSumText.textContent =
        "минимальное кол-во занятий в неделю - 2, максимальное - 5";
  ...
  targetlList.onchange = function () {
    if (targetlList.value == 1) {
      ...
      formForSend.target =
        "Ваша цель - получить помощь с домашними заданиями по школе";
      ...
      formForSend.target =
        "Ваша цель - не знаю, но изучать английский сейчас модно";
...
function calculatePrice() {
  sumOfObjElem =
    sumElemObj.selectedCourse +
    4000 * sumElemObj.monthSum * sumElemObj.lessonsSum;
...
function buttonNextUnlock() {
  if (
    Object.keys(formForSend).length == 5 &&
    firstPopValid.courseList == true &&
    firstPopValid.lessonsInput == true &&
    firstPopValid.targetList == true
  ) {
...
function secondFormPoP() {
  ...
  let scanEmalePopWind = setInterval(catchEmlFormValue, 10);
  ...
  arrowBackTxt.addEventListener("click", function () {
    backToFirst();
  });
  ...
  emlFootHeader = document.createElement("li");
  emlFooterText.appendChild(emlFootHeader);
  emlFootHeader.classList.add("emlFootHeader");
...
function catchEmlFormValue() {
  ...
  if (
    Object.keys(emlForm).length == 4 &&
    emlValidFlag.emailV == true &&
    emlValidFlag.phoneV == true
  ) {
    emlFootHeader.textContent = "Ваша заявка будет отправлена менеджеру!";
...
function writeEmlInpValue() {
  ...
  if (checkWroteEmlInp(1) && emlFlagForBack == true) {
    ...
    let scanEmlFormPopUp = setInterval(catchEmlFormValue, 10);
